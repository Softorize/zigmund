...
- OpenAPI hardening:
  ...
  - route/websocket OpenAPI security declarations now support explicit OR-alternative requirement arrays (`RouteOptions.openapi_security`, `WebSocketRouteOptions.openapi_security`) with override precedence over dependency-derived defaults.
...
- Runtime floor hardening (M1 progress):
  ...
  - Server loop now supports shutdown signaling + connection drain behavior, and new connections during drain receive explicit `503 Service Unavailable` shutdown responses (with overload retry policy applied).
...
- Validation/serialization parity progress (M2 slice):
  ...
  - response models can now define model-level post-shaping validation hooks (`zigmund_response_validate`) for deterministic output invariant enforcement.
...
- New conformance coverage added for:
  ...
  - CLI renderer coverage for routes JSON and cloud-plan output shape (including HTTP/websocket policy metadata fields and dependency-detail arrays),
